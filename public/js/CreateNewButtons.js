 ...
 * @returns
 */
function createEditor (textarea_id, createPrint) {
...
//		Beim Einfuegen werden Formate geloescht
		...
		paste_word_valid_elements: "p,b/strong,ul,ol,li,br,br\/,h2/h1,h2,b/h3,b/h4,b/h5,b/h6,div[align]",
		valid_elements: "p,span[class],b/strong,ul,ol,li,br,br\/,h2/h1,h2,b/h3,b/h4,b/h5,b/h6,div[align]",
...
//      Erstellt die obere Toolbar und fuegt Buttons ein
        toolbar1: "clearContent | h2 | bold | bullist | alignleft aligncenter alignright | delete_mark",// | text_check_" + textarea_id,

//      Erstellt die untere Toolbar und fuegt Buttons ein
        toolbar2: "printPDF printSinglePDF",
            ...
            */
            ...
            ed.addButton('clearContent', {
                ...
                onclick: function(){
                    ...
                    if(!window.confirm('Möchten Sie wirklich den gesamten Inhalt des Editors löschen ?'))
                        return;
                    ...
                    if($('.pdfForm').length)
                    {
                        ...
                        $('.pdfForm input[name="deckpage"][value="1"]').prop('checked', true);

                        $('.pdfForm input[name="lastpage"]').prop('checked', false);
                        $('.pdfForm input[name="lastpage"][value="1"]').prop('checked', true);

                        $('.pdfForm input[name="title"]').val('');
                        $('.pdfForm input[name="company"]').val('');
                        $('.pdfForm input[name="contact"]').val('');
                        $('.pdfForm input[name="id"]').val(0);

                        $('.pdfForm select[name="footer"] option').prop('selected', false);
                        $('.pdfForm select[name="footer"] option[value="© 2015 - 2016 www.stellenanzeigen-texten.de powered bei aicovo gmbh"]').prop('selected', true);
                    }
                }
            });
...
//              Bei einem Klick, wird die Funktion zum wiederholen mit dem Klassenamen des Editors aufgerufen
            ...
            ed.on('keyup', function(e) {
                var intTextLength   = ed.getContent().length;
